...
/// Body for `POST /marketplace/offers` (broadcast a storage offer).
#[derive(Deserialize)]
struct ProposeStorageBody {
    network_id: String,
    bytes_offered: u64,
...
/// `POST /marketplace/offers` — broadcast a storage offer.
///
/// Body: `{ "network_id": "public", "bytes_offered": 1073741824 }`
#[instrument(skip(state, body))]
async fn marketplace_propose(
    State(state): State<AppState>,
    Json(body): Json<ProposeStorageBody>,
) -> Response {
    daemon_call!(
        state,
        ControlRequest::ProposeStorage {
            network_id: body.network_id,
            bytes_offered: body.bytes_offered,
            duration_secs: body.duration_secs,
            price_tokens: body.price_tokens,
        }
    )
}

/// Query params for `GET /marketplace/offers` and `GET /marketplace/agreements`.
#[derive(Deserialize)]
...
/// `GET /marketplace/offers` — list received storage offers.
...
/// `POST /marketplace/accept` — accept a storage offer.
///
/// Body: `{ "offer_id": "<uuid>" }`
#[instrument(skip(state, body))]
async fn marketplace_accept(
    State(state): State<AppState>,
...
/// `GET /marketplace/agreements` — list local storage agreements.
#[instrument(skip(state, q))]
async fn marketplace_list_agreements(
    State(state): State<AppState>,
    Query(q): Query<MarketplaceQuery>,
) -> Response {
    daemon_call!(
